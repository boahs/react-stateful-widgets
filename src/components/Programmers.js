...
export default function Programmers() {
  ...
  const [programmers, getProgrammers] = useState(listOfAwesome); /*listOfAwesome.name*/
  const [programmersID, featuredProgrammer] = useState(null); /*listOfAwesome.id*/
  ...
  const style = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: programmersID ? "yellow" : "royalblue", // 🤔
  };
      ...
      <div className="programmers">
        {
          ...
          programmers.map((dev) => (
            <div key={dev.id}>
              ...
              <button
                onClick={() => {
                  featuredProgrammer(dev.id); getNameOfFeatured()
                }}
              >
                Feature
              </button>
